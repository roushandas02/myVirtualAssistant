...
function Signup() {
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bg})`}}>
        < form className='p-[20px] w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px]' onSubmit={handleSignup}>
            ...
            <button className='btn btn-primary min-w-[150px] h-[60px] mt-[30px] font-semibold rounded-full text-[19px] cursor-pointer'disabled={loading}>{loading?"Loading...":"Sign Up"}</button>
            <p className='text-[white] text-[18px] '>Already have an account? <span className='text-blue-400 cursor-pointer' onClick={()=>navigate("/login")}>Log In</span></p>
        </form>
    </div>
  )
}
